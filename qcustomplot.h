...
public:
  ...
  */
  ...
  void changeColorStopAt(double position, const QColor &color);
...
{
  void updateDataBoundsAfterInsert(bool top, double newMin, double newMax, double oldMin, double oldMax);
...
public:
  ...
  void insertValues(QVector<double> values, bool top);
...
enum InterpolationMode{
    imClassic, //классическая отрисовка
    imCustom //с 3х точечным сглаживанием по оси x, методом ближайшего соседа по оси y
};
...
class QCP_LIB_DECL QCPColorMap : public QCPAbstractPlottable
{
  ...
  /// \endcond
  QImage applyCustomInterpolation(const QImage &sourceImage); //
  QRgb interpolatePixels(QRgb color1, QRgb color2, float alpha);
  bool isSimilarColor(QRgb color1, QRgb color2);
  int mOutlierThreshold;

public:
  explicit QCPColorMap(QCPAxis *keyAxis, QCPAxis *valueAxis);
  virtual ~QCPColorMap() Q_DECL_OVERRIDE;
  void setInterpolationMode(InterpolationMode mode);
  InterpolationMode interpolationMode() {return mInterpolationMode;}

  // Порог для детекции выбросов (только для кастомной интерполяции)
  int outlierThreshold() const { return mOutlierThreshold; }
  void setOutlierThreshold(int threshold);
  ...
  QImage getMapImage() const { return  mMapImage;}
